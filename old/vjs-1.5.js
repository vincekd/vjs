function $ ( i ) {
    return { "$" : ( typeof i == "string" ) ? document.getElementById( i ) : i,
		 ...
		 for ( a in p ) {
		     this.$.setAttribute ( a, p[a] );
		 }
	     },
	     "fadeIn" : function ( n, s ) {
		 fadeIn ( this.$, n, s );
	     },
	     "fadeOut" : function ( n, s ) {
		 fadeOut ( this.$, n, s );
	     },
	     "class" : function ( m ) {
...
function fI ( ){
    ...
    if ( o < n ) {
	if (  window[i + "-t"] == null ){
	    window[i + "-t" ] = setInterval ( "fI(\"" + i +"\", \"" + n + "\", \"" + s + "\")", 10 );
	}
    } else {
	clearInterval ( window[i + "-t"] );
	window[i + "-t"] = null;
    }
};
function fadeOut ( i, n, s ) {
    if ( n == undefined ){ n = 0;}
    if ( s == undefined ){ s = .01;}
...
function fO ( ){
    ...
    if ( o > n ) {
	if (  window[i + "-t"] == null )
	    window[i + "-t"] = setInterval ( "fO(\"" + i +"\", \"" + n + "\", \"" + s + "\")", 10 );
    } else {
	clearInterval ( window[i + "-t"] );
	window[i + "-t"] = null;
